...
export function Navbar() {
  ...
  const activeClass = useLightText ? 'text-orange-300 font-semibold' : 'text-primary-700 font-semibold';

  function NavLink({ item }: { item: { path: string; label: string } }) {
    const isActive = location.pathname === item.path || (item.path !== '/' && location.pathname.startsWith(item.path));
    return (
      <Link to={item.path} className={`relative py-6 font-body text-sm font-medium transition-colors whitespace-nowrap ${isActive ? activeClass : linkClass}`}>
        {item.label}
        {isActive && (
      ...
      <nav className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        <div className="flex items-center justify-between min-h-[68px] lg:min-h-[76px]">
          ...
          <Link to="/" className="flex items-center gap-3 group shrink-0">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-heading font-bold text-lg transition-all duration-200 group-hover:scale-105 ${useLightText ? 'bg-white/15 text-white ring-2 ring-white/30' : 'bg-primary-900 text-white'}`}>
              SP
            </div>
            <div className="hidden sm:block">
              <div className={`font-heading font-bold text-base leading-tight ${useLightText ? 'text-white' : 'text-primary-900'}`}>
                Shubham Properties
              </div>
              <div className={`font-body text-xs ${useLightText ? 'text-white/70' : 'text-slate-500'}`}>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((item) =>
              'children' in item ? (
                ...
                >
                  <button className={`flex items-center gap-1 px-3 py-6 font-body text-sm font-medium transition-colors whitespace-nowrap ${location.pathname.startsWith('/services') ? activeClass : linkClass}`}>
                    {item.label}
